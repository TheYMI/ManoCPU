 ...
 * Date: 14/12/12
 * Time: 21:27
 * Description: This is the register class. It defines all the methods and members each register must have.
 *              All registers will inherit from this class. A register is a component.
 *              Members defined by this class are the register's value and the write enable flag.
 *              Each register has a value buffer to mimic the master-slave functionality of real flip-flops.
 *              Values are set to the buffer and read from the value. An update method will move the value from the buffer to the value.
 *              Values can be set only if the write enable flag is true.
 * ***********************
 */

public abstract class Register extends Component implements DataComponent {
    ...
    public void set_value(Value value){
        if (mWriteEnable){              // Write only if writing is enabled.
            mBuffer.set_content(value); // Write to the buffer to mimic master-slave functionality.
        }
    }

    // Getter:
    public Value get_value(){
        return mValue; // Read the current value.
    }
    ...
    // Enables the writing to the register.
    // Parameters:
    // Return value:
    ...
    // Disables the writing to the register.
    // Parameters:
    // Return value:
    ...
    // Checks if writing to the register is enabled.
    // Parameters:
    ...
    // Sets the register value to 0.
    // Parameters:
    // Return value:
    public void clear(){
        mBuffer.set_content(0);
    }
    ...
    // Updated the value of the register to the value of the buffer.
    // Parameters:
    // Return value:
    public void update(){
        mValue.set_content(mBuffer.get_content());
    }

    // Sets the value in the bit of the register at the index given.
    // Parameters: Index and value.
    // Return value:
    public void set_value(int index, Value value){

        boolean[] newValue = mValue.get_content();

        newValue[(mValue.get_size() - 1) - index] = value.get_content()[0];
        set_value(new Value(newValue));
    ...
    // Sets the value of the bits in the range, starting with the bit given. End bit is ignored, and exists for consistency.
    // Parameters: Starting index, value.
    // Return value: The value of the bits range.
    public void set_value(int start, int end, Value value){

        boolean[] binVal = value.get_content();
        boolean[] regVal = mValue.get_content();

        start = (regVal.length - binVal.length - start);

        for (int i = start, j = 0; (i < regVal.length) && (j < binVal.length); ++i, ++j){
            if (i >= 0){
                regVal[i] = binVal[j];
            }
        }

        set_value(new Value(regVal));
